...
import { KEYS, cacheDelMany } from "../../../lib/cache.js";
...
const invalidateDashbord = (userId) => cacheDelMany([KEYS.todosDashbord(userId), KEYS.dashboard(userId)]);
...
export const updateTodoStatus = async (req, res) => {
    try {
        ...
        const todo = await todoModel.findOneAndUpdate(
            { _id: id, userId },
            { isCompleted, status: isCompleted ? 'completed' : (isExpired ? 'expired' : 'pending'), isExpired },
