...
const expireForTimezones = async (timezones) => {
    let totalExpired = 0;
...
const expireOverdueTodos = async () => {
    ...
    try {
        ...
        let totalExpired = await expireForTimezones(timezones.filter(Boolean));
        ...
        if (legacyUserIds.length > 0) {
            const legacyResult = await todoModel.updateMany(
                {
                    userId: { $in: legacyUserIds },
                    isCompleted: false,
                    isExpired: false,
                    dueDate: { $lt: new Date(todayStartForTimezone(DEFAULT_TIMEZONE)) }
                },
                { $set: { isExpired: true, status: 'expired' } }
            );
            totalExpired += legacyResult.modifiedCount;
        }
