...
const calculateDashboard = async (userId) => {
    const user = await userModel.findById(userId).select("timezone activeTimetableId").lean()
    ...
    const upcomingTodosPromise = todoModel.find({
        userId,
        dueDate: {
            $gte: todoStart,
            $lt: todoEnd
        ...
        isCompleted: false,
        isExpired: false
    }).select("title priority dueDate isCompleted").lean()
    ...
    let todayClasses = []
    let attendanceLogsPromise = null
    ...
    if (timetable) {
        ...
        if (isWithinTimetable) {
            const classes = await classModel.find({
                ...
                isActive: true
            }).select("name startTime endTime color timeTableId").lean()

            attendanceLogsPromise = attendanceLogModel.find({
                userId,
            ...
            todayClasses = { classes, timetableId: timetable._id }
        }
    }

    const [attendanceLogs, upcomingTodos] = await Promise.all([
        attendanceLogsPromise,
        upcomingTodosPromise
    ])

    if (todayClasses?.classes?.length) {
...
export const fetchDashboard = async (req, res) => {
    try {
        ...
        const REDIS_KEY = KEYS.dashboard(userId)
        const cached = await cacheGet(REDIS_KEY)
        if (cached) {
            return res.status(200).json({
                success: true,
                message: MESSAGE.redisMss,
                result: JSON.parse(cached)
            })
        }

        const result = await calculateDashboard(userId)

        if (!result) {
            return res.status(400).json({ success: false, message: MESSAGE.userErr })
        }

        await cacheSet(REDIS_KEY, JSON.stringify(result), 60)
