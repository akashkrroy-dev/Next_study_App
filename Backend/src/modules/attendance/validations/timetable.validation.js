...
const subject_create_schema = z.object({
    ...
    classes: z.array(day_enum).optional()
})
...
export const create_timetable_schema = z.object({
    ...
    { message: "endDate must be greater than startDate", path: ["endDate"] }
)
...
const update_timetable_op = z.object({
    ...
    (data) => !data.endDate || !data.startDate || data.endDate.getTime() > data.startDate.getTime(),
    { message: "endDate must be greater than startDate", path: ["endDate"] }
)
...
const update_cls_op = z.object({
    ...
    (data) => !data.update.startTime || !data.update.endTime || data.update.startTime < data.update.endTime,
    { message: "startTime must be before endTime", path: ["update", "endTime"] }
)
