...
import { KEYS, cacheGet, cacheSet, cacheDel, cacheDelMany } from "../../../lib/cache.js";
...
export const todayClasses = async (req, res) => {
  try {
    ...
    const attendanceByClassId = new Map(attendanceLogs.map((log) => [log.classId.toString(), log]))

    const todayClasses = classes.map((cls) => ({
      ...cls.toObject(),
      attended: attendanceByClassId.get(cls._id.toString())?.isAttend === true,
    }))

    return res.status(200).json({
      success: true,
      message: "Fetched Today Classes!",
      todayClasses
    })
  ...
  try {
    ...
    if (attendanceLog) {
      ...
      try {
        ...
        // Duplicate key: a concurrent request already created a log for this
        // class/day. Reconcile by updating instead of failing or duplicating.
        if (error?.code === 11000) {
          attendanceLog = await attendanceLogModel.findOneAndUpdate(
            {
              userId,
              classId: clsId,
              subjectId: classDoc.subjectId,
    ...
    const gridMonth = today.month - 1
    const gridYear = today.year

    await cacheDelMany([
      KEYS.gridData(userId, ttId, gridMonth, gridYear),
    ...
    return res.status(created ? 201 : 200).json({
      success: true,
      message: created ? "Attendance marked" : "Updated attendanceLog",
      attendanceLog
    })
...
export const targetAttendance = async (req, res) => {
  try {
    ...
    const REDIS_KEY = KEYS.target(userId, id)

    const data = await cacheGet(REDIS_KEY)
    if (data) {
      const subjects = JSON.parse(data)
      return res.status(200).json({ success: true, message: "fetch from redis", subjects })
    }
    ...
    if (_subjects.length === 0) {
      return res.status(200).json({ success: true, message: "Add subjects to see target data", subjects: [] })
    }
    ...
    const earliestStart = allVersions.reduce((earliest, versionDoc) => {
      const versionStart = Temporal.PlainDate.from(versionDoc.startDate.toISOString().split("T")[0])
      return !earliest || Temporal.PlainDate.compare(versionStart, earliest) < 0
        ? versionStart
        : earliest
    }, null)
    ...
    const logs = await attendanceLogModel.find({
      ...
      date: {
        $gte: earliestStart ? plainDateToDate(earliestStart) : timetable.startDate,
        $lte: plainDateToDate(lastCounted)
      }
    }).lean()
    ...
    const result = _subjects.map((sub) => {
      ...
      if (totalHeld > 0) {
        if (currentPercent >= targetValue) {
          ...
          // JSON cannot represent Infinity; use a sentinel the frontend maps to
          // "must attend every remaining class".
          mustAttend = 1_000_000
        } else {
...
export const gridDataDisplayInMonth = async (req, res) => {
  try {
    ...
    const REDIS_KEY = KEYS.gridData(userId, id, month, year)
    ...
    const result = _subjects.map((sub) => {
      const cells = days.map((day) => {
        ...
        if (isHoliday(plainDate)) {
          status = "holiday"
        } else if (endPlain && Temporal.PlainDate.compare(plainDate, endPlain) > 0) {
          status = "not-scheduled"
        } else if (Temporal.PlainDate.compare(plainDate, startPlain) < 0) {
          status = "not-scheduled"
    ...
    await cacheSet(REDIS_KEY, JSON.stringify(responsePayload))
