...
export const createTimeTable = async (req, res) => {
  try {
    ...
    await cacheDel(KEYS.dashboard(userId))
...
export const updateTimetable = async (req, res) => {
  try {
    ...
    const resolvedStart = _updates.startDate || timetable.startDate
    const resolvedEnd = _updates.endDate !== undefined ? _updates.endDate : timetable.endDate
    ...
    if (!isMezor) {
      ...
      await runWithTransaction(async (session) => {
          ...
          user.activeTimetableId = newTimeTable._id
  ...
  try {
    ...
    for (const op of updates) {
      ...
      if (op.type === 'update_cls') {
        ...
        const dayChanged = update.day !== undefined && update.day !== existingCls.day
        const subjectChanged = update.subjectId !== undefined && update.subjectId !== existingCls.subjectId

        if (dayChanged || subjectChanged) {
          const oldSubject = _subjects.find((s) => s.subjectId === existingCls.subjectId)
          if (oldSubject) {
            const idx = oldSubject.classes.indexOf(existingCls.day)
            if (idx !== -1) oldSubject.classes.splice(idx, 1)
          }

          const targetSubjectId = subjectChanged ? update.subjectId : existingCls.subjectId
          const targetDay = dayChanged ? update.day : existingCls.day
          const targetSubject = _subjects.find((s) => s.subjectId === targetSubjectId)
          if (targetSubject) targetSubject.classes.push(targetDay)
...
export const slugToCopyTimeTable = async (req, res) => {
  try {
    ...
    // 7. Copy every class
    const classes_copy = classes
      .map((cls) => {
        const newSubjectId = subjectIdMap.get(cls.subjectId);
        if (!newSubjectId) return null;
    ...
    // 8. Only switch the active timetable once the copy is fully in place
