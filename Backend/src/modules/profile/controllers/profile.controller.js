...
import { clearRefreshTokenCookieOptions } from "../../../utils/cookieOptions.js";
import { KEYS, cacheDelMany, cacheDelPattern } from "../../../lib/cache.js";
  ...
  try {
    ...
    for (const [updateKey, updateObj] of Object.entries(updates)) {
      ...
      if (updateKey === 'reminders' || updateKey === 'notifications') {
        await settingsModel.findByIdAndUpdate(settings._id, { $set: updateObj }, { returnDocument: 'after' });
    ...
    try {
      if (timezoneChanged) {
        const userTimetables = await timetableModel.find({ userId, isActive: true }).select("_id").lean()
        await cacheDelPattern(KEYS.gridData(userId, "*", "*", "*"))
        await cacheDelMany([
          KEYS.dashboard(userId),
          KEYS.todosDashbord(userId),
          ...userTimetables.flatMap((tt) => [
            KEYS.totalDashbord(userId, tt._id),
            KEYS.target(userId, tt._id),
            KEYS.timetable(userId, tt._id),
          ]),
        ])
      }
