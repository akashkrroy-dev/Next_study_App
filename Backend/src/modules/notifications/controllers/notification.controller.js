...
export const getNotifications = async (req, res) => {
    try {
        ...
        return res.status(500).json({
            success: false,
            message: "Server Error at Notification Fetch route"
        })
    }
}
...
export const setDeletedNotification = async (req, res) => {
    try {
        const userId = req.userId
        const { id } = req.params
        if (!id) { return res.status(400).json({ success: false, message: "id Required!" }) }
...
export const pushAppUpdateNotification = async (req, res) => {
    try {
        ...
        const { title, message, type = "announcement" } = req.validatedBody
        ...
        return res.status(500).json({
            success: false,
            message: "Server Error at Global Notification Route"
        })
    }
}